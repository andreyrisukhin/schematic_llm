...
def id_to_block(block_ids):
    ...
    for i,b in enumerate(block_ids):
        # print(f'{str(i)}: {MAP.get(str(i))}')
        block = MAP.get(str(b) + ":0", MAP.get("0:0"))
        # print(block)
        blocks.append(block)
    
        # if (b,block) in inspect:
        #     inspect[(b,block)].append(i)
        # else:
        #     inspect[(b,block)] = [i]

    # Deduplicate inspect based on b, block (ignore i)
